...
const Image1 = require('../../assets/FrameBPart4.png')
const ModalCancel = require('../../assets/modalCancel.png')

export default class FrameBPart4 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      fontSizeIncrement: 0,
        ...
        <ScrollView style={styles.ModalCover}>
          <View style={styles.Image1Cover}>
            <ImageZoom
              ...
              imageHeight={(WindowWidth - (2 * WindowWidth / 60)) * 0.22}
            >
      ...
      <SafeAreaView style={{ flex: 0, backgroundColor: '#5C3B96F7' }} />
        ...
        <ScrollView style={styles.Body} stickyHeaderIndices={[1]} showsVerticalScrollIndicator={false}>
          <View style={styles.ContentHeadingCover}>
            <Text style={[
              styles.ContentHeading, { fontSize: (WindowWidth / 21) + this.state.fontSizeIncrement }]}>{data.FrameB.sections[3]}
            </Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  Image1: {
    ...
    height: (WindowWidth - (2 * WindowWidth / 60)) * 0.22,
    alignSelf: 'center'
  },
  CancelButtonCover: {
    position: 'absolute',
